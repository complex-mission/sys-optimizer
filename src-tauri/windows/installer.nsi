...
; ── 本项目对官方模板(tauri-cli 2.11.4)的修改 1/2 ──
; 移除上游的 `ManifestDPIAwareness PerMonitorV2`:NSIS 不处理 WM_DPICHANGED,
; PMv2 的对话框自动缩放会把单选框/复选框图元画出锯齿(自定义维护页尤甚)。
; 仅保留 System-DPI 感知:主显示器缩放下渲染清晰,跨显示器由系统拉伸兜底。

; ── 修改 2/2 ──
; 统一安装器字体:NSIS 简体中文语言包默认宋体,与版权栏(BrandingText)等
; 控件字体不一致;显式 SetFont 后所有对话框(含品牌栏)同用雅黑,中英文皆可。
...
!if "${SIDEBARIMAGE}" != ""
  ...
  ; Also use it for the uninstaller welcome/finish pages, which otherwise
  ; fall back to the default NSIS wizard bitmap
...
Function .onInit
  ...
  ; Desktop shortcut name follows installer language.
  ; The localized name is defined in hooks.nsh (kept there for UTF-8 BOM).
  StrCpy $ShortcutName "${PRODUCTNAME}"
  !ifdef SHORTCUTNAME_ZH
    ${If} $LANGUAGE == 2052 ; SimpChinese
      StrCpy $ShortcutName "${SHORTCUTNAME_ZH}"
    ${EndIf}
  !endif
